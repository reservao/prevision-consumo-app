...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body::before {
    ...
    inset: 0;
    background:
      radial-gradient(ellipse at 20% 20%, rgba(31,78,121,0.08) 0%, transparent 60%),
...
<div class="card">
  <div class="logo-row">
    ...
    <div class="logo-text">
      ...
      <span>Transformador de datos</span>
    </div>
  </div>
  ...
  <div class="steps">
    <div class="step">
      ...
      <div class="step-label">Sube el .xls</div>
    </div>
    <div class="step">
      <div class="step-num">⚡</div>
      <div class="step-label">Procesamos</div>
    </div>
    <div class="step">
  ...
  <form id="uploadForm">
    ...
    <button class="btn" type="submit" id="submitBtn" disabled>
      Transformar y descargar Excel
    </button>
  </form>

  <div class="loading" id="loading">
    <div class="spinner"></div>
    Procesando tu archivo, un momento…
  </div>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = 'Prevision_Consumo_Tabla.xlsx';
        a.click();
...
def transformar_archivo(filepath):
    ...
    for idx, marker in enumerate(unit_markers):
        ...
        while i < end:
            ...
            cantidad = df.iloc[i, 3]
            unidad = str(df.iloc[i, 8]).strip() if len(df.columns) > 8 else ''
            nombre = ''
            if i + 1 < end:
                nombre = str(df.iloc[i + 1, 0]).strip()
    ...
    ws['A2'] = f'Total registros: {len(result)}  |  Unidades Agregadas: {result["ID Unidad Agregada"].nunique()}  |  Generado: {datetime.now().strftime("%d/%m/%Y %H:%M")}'
    ...
    for _, rec in result.iterrows():
        unit_id = rec['ID Unidad Agregada']
        if unit_id != prev_unit:
            ws.merge_cells(f'A{row_num}:F{row_num}')
            cell = ws.cell(row=row_num, column=1, value=f"  Unidad Agregada: {unit_id} – {rec['Nombre Unidad Agregada']}")
